...
export const getStavSpravyFunction = async(e, id_odosielatelaSpravy) =>{
  if (e?.preventDefault) e.preventDefault();
  const apiUrl = `http://192.168.0.170:5000/api/stav/spravy`; 
  try {
    const response = await fetch(`${apiUrl}/${id_odosielatelaSpravy}`);
    if (!response.ok) {
      throw new Error('Failed to fetch items');
    }
    const data = await response.json();
